...
class WritableStore<T> implements Writable<T> {
    ...
    // Subscribe method for reactive stores
    subscribe(run: Subscriber<T>): Unsubscriber {
        run(this.value); // Call immediately with the current value
    ...
    // Get method to access the current value
    get(): T {
        return this.value;
    }

    // Set method to update the value and notify all subscribers
    set(newValue: T): void {
    ...
    // Update method to modify the current value based on a function
...
const writable = <T>(initialValue: T): WritableStore<T> => new WritableStore(initialValue);
...
export class DerivedCollectionController {
	...
	public mappedOrigins: Writable<originRowData[]> = writable<originRowData[]>([]);

	public generativeNameListData : Writable<string[]>	= writable<string[]>([]); 
	...
	public onKeyDelete( e ){ 
		this.mappedOrigins.update( mappedOrigins => {
			...
			if (!old.active || !old.inCalc){
				mappedOrigins = mappedOrigins.filter(p => p != old );
			} else {
				old.active = false;
